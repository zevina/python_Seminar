...
def add_employees():
    print("Введите данные сотрудника (ФИО, телефон, должность) через пробел:")
    data = input().split()
    return data
...
def change_employees():
    ...
    change = int(input())
    print("Введите новые данные сотрудника (ФИО, телефон, должность) для перезаписи через пробел:")
    string = input().split()
